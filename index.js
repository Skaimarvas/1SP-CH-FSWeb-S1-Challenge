...
function indekseGoreFenomen(feno, indo) {
  return `${indo}. indekste bulunan fenomen: ${feno[indo].profile}`;
}

console.log("Görev 3", indekseGoreFenomen(fenomenler, 4));
...
function profilListesi(hort) {
  const yeniHort = [];
  for (let i = 0; i < hort.length; i++) {
    yeniHort.push(hort[i].profile);
  }
  return yeniHort;
}
...
function fenomenSil(Aloho, Mora) {
  const newAloho = Aloho;
  newAloho.splice(Mora, 1);
  return newAloho;
}
...
function fenomenEkle(v, vi, veri, veniversum, vivus, vici) {
  const newArr = v;
  const addOb = {
    number: vi,
    profile: veri,
    followers: veniversum,
    posts: vivus,
    platform: vici,
  };
  newArr.push(addOb);
...
function enFenomenler(expecto) {
  const mostFol = [];
  for (let i = 0; i < expecto.length; i++) {
    const element = expecto[i].followers;
    if (element > 100000000) {
      mostFol.push(expecto[i].profile);
    }
  }
  return mostFol;
}
